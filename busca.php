...
<?php
$nome = $_POST['nome'];
$animes = DBRead( 'animes', "WHERE id and upper(nome) LIKE upper('%{$nome}%') ORDER BY id DESC LIMIT 5" );
$resultsearchs = DBRead( 'animes', "WHERE id and upper(nome) LIKE upper('%{$nome}%') ORDER BY id DESC LIMIT 5" );
 if (!$resultsearchs)
 echo '';
else
...
?>
<?php
$animes = DBRead( 'videos', "WHERE id and id_anime = ".$resultsearch['id']." ORDER BY id DESC LIMIT 5" );
$resultsearchs2 = $animes = DBRead( 'videos', "WHERE id and id_anime = ".$resultsearch['id']." ORDER BY id DESC LIMIT 5" );
 if (!$resultsearchs)
 echo '';
else
...
$sql = $db->con()->prepare("SELECT * FROM ani_animes WHERE nome LIKE '%$nome%' LIMIT 9");
$sql->execute();
...
if(empty($nome)){
	...
	while($ftc = $sql->fetchObject()){
		echo "<a href='http://localhost/watch.php?id=".$resultsearch2['id']."'><li id='birlada'>"."<img id='searchimg' src='http://127.0.0.1/img/animes/".$ftc->foto."' />"."<div class='name'><p class='name'>".$ftc->nome."</p></div>".""."</li></a>";
	}
}
?>
